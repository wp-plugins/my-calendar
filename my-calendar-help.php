...
<?php _e('The shortcode supports these attributes:','my-calendar'); ?>
	...
	<li><code>above</code>, <code>below</code>: <?php _e("Comma-separated list of navigational features to display above or below the calendar. Available: <strong>nav, toggle, jump, print, key, feeds</strong>. Order listed determines the order displayed. Feed links and print view will only display if they are enabled on the settings page. All other fields will appear if specified; the defaults specified in settings will be used if the attribute is left blank. Use <em>none</em> to hide all navigation elements.",'my-calendar'); ?></li>
