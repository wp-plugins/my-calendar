  ...
  // This function cannot be called unless calendar is up to date
	...
	if ($display_upcoming_type == "days") {
		$temp_array = array();
			...
			$events = my_calendar_grab_events( $from, $to, $category,'','','upcoming' );
			//$current_date = "$y-$m-$d";
		...
		if ( $caching ) { 
			...
			if ( $cache ) {
				if (isset($cache[$category]) ) {
					...
					$events = mc_get_all_events($category, $before, $after, $show_today);
			...
			$events = mc_get_all_events($category, $before, $after, $show_today);	 // grab all events within reasonable proximity
		}
